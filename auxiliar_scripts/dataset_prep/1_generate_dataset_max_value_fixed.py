...
def main():
    ...
    PATH_TRAIN = ['Area_1', 'Area_6_tif_inteiro', 'Area_7']
    PATH_VAL = ['Area_4']
    PATH_TEST = ['Area_3']

    EXCLUDE_IMAGES = ['20211029_140029_54_2402_3B_AnalyticMS_SR.tif', '20211029_140027_25_2402_3B_AnalyticMS_SR.tif', '20211008_135132_57_227a_3B_AnalyticMS_SR.tif', '20211008_130249_09_2428_3B_AnalyticMS_SR.tif', '20211008_135130_27_227a_3B_AnalyticMS_SR.tif', '20211008_135127_96_227a_3B_AnalyticMS_SR.tif', '20211008_130246_61_2428_3B_AnalyticMS_SR.tif']
    ...
    for path in PATH_TRAIN:
        tifs += [os.path.join(BASE_PATH, path, 'tifs', f) for f in os.listdir(os.path.join(BASE_PATH, path, 'tifs')) if f.endswith(tuple(extension)) and f not in EXCLUDE_IMAGES]
    ...
    for i, tif in enumerate(tifs):
        print(f'{i}/{size} - Processing: {tif}\n')
        args.split = 'val'
        val_file_names += generate(tif, shps, OUT_PATH, slice_size, step, args)

    n_patches = len(val_file_names)
    with open(os.path.join(OUT_PATH, 'ImageSets/Segmentation', args.split+'.txt'), 'a') as f:
            f.write('\n'.join(val_file_names))
    ...
    for i, tif in enumerate(tifs):
        print(f'{i}/{size} - Processing: {tif}\n')
        args.split = 'test'
        test_file_names += generate(tif, shps, OUT_PATH, slice_size, slice_size, args) # no overlap
    
    n_patches = len(test_file_names)
    with open(os.path.join(OUT_PATH, 'ImageSets/Segmentation', args.split+'.txt'), 'a') as f:
