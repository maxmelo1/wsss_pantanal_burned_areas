...
class Random_Erase:
    def __init__(self, max_size, channels=3):
        self.max_size = max_size
        self.bg_value = 0
        # self.crop_size = crop_size
        # self.crop_shape = (self.crop_size, self.crop_size)

    def __call__(self, image):
        h, w, c = np.array(image).shape

        max_size = self.max_size-1 if self.max_size < h and self.max_size<w else min(h,w)

        crop_size = np.random.randint(0, max_size)

        cx = np.random.randint(0, h-crop_size)
        cy = np.random.randint(0, w-crop_size)

        # print(crop_size)

        cropped_image = np.copy(image)
        cropped_image[cx:cx+crop_size, cy:cy+crop_size] = 0
