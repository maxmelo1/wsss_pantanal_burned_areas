...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        for step, (ori_image, image_id, gt_mask) in enumerate(dataset):
            ...
            if args.iteration > 0:
                # h, w, c -> c, h, w
                preds = crf_inference(np.asarray(ori_image), preds.transpose((2, 0, 1)), t=args.iteration, labels=2)
            ...
            pred_mask_p = Image.fromarray(pred_mask.astype(np.uint8)).convert('P')
            pred_mask_p.putpalette(dataset.colors[...,::-1])
            pred_mask_p.save(pred_dir + image_id + '.png')
