...
class Backbone(nn.Module, ABC_Model):
    def __init__(self, model_name, num_classes=20, mode='fix', segmentation=False):
        ...
        if 'resnet' in model_name:
            block = resnet.BasicBlock if model_name in ['resnet18', 'resnet34'] else resnet.Bottleneck
            self.model = resnet.ResNet(block, resnet.layers_dic[model_name], strides=(2, 2, 2, 1), batch_norm_fn=self.norm_fn)
        ...
        weights = self.model.conv1[0].weight.data.clone()
        
        #new_conv1 = nn.Conv2d(4, 64, kernel_size=7, stride=2, padding=3, bias=False)
        new_conv1 = nn.Conv2d(4, 64, kernel_size=3, stride=2, padding=1, bias=False)
        #new_conv1 = nn.Conv2d(4, 32, kernel_size=3, stride=2, padding=1, bias=False) #resnest50

        with torch.no_grad():
            new_conv1.weight[:,:3] = weights
            new_conv1.weight[:,-1] = weights[:,-1]
        # print('4 canal->', new_conv1.weight[:, 3])
        self.model.conv1[0] = new_conv1
...
class Classifier(Backbone):
    ...
    def forward(self, x, with_cam=False):        
        ...
        f_copy = f_copy.cpu().detach().numpy()
        # print(f_copy)
