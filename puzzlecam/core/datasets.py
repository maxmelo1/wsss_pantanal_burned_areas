...
class VOC_Dataset(torch.utils.data.Dataset):
    ...
    def get_image(self, image_id):
        # image = Image.open(self.image_dir + image_id + '.jpg').convert('RGB') ### max
        image = Image.fromarray(np.load(self.image_dir + image_id + '.npy'))
        # image = Image.fromarray(np.load(self.image_dir + image_id + '.npy')[:,:, :3])
        return image
