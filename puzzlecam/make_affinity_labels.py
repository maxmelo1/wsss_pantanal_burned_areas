...
if __name__ == '__main__':
    ...
    for step, (ori_image, image_id, _, _) in enumerate(dataset):
        ...
        # print(image_id, label, np.unique(cams, return_counts=True))

        # 1. find confident fg & bg
        fg_cam = np.pad(cams, ((1, 0), (0, 0), (0, 0)), mode='constant', constant_values=args.fg_threshold)        
        fg_cam = np.argmax(fg_cam, axis=0)
        #print(image_id, '->', keys.shape[0], '->', np.shape(fg_cam[fg_cam>0]))
        #mudei aqui
        lbls = keys.shape[0] if keys.shape[0] > 1 else 2
        #fg_conf = keys[crf_inference_label(image, fg_cam, n_labels=keys.shape[0])]
        fg_conf = keys[crf_inference_label(image, fg_cam, n_labels=lbls)]
        ...
        bg_conf = keys[crf_inference_label(image, bg_cam, n_labels=lbls)]
        ...
        # print(np.unique(conf, return_counts=True))
        # input()
