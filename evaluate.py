...
def compare(start,step,TP,P,T, name_list):
    for idx in range(start,len(name_list),step):
        ...
        if os.path.isfile(predict_folder + name + '.npy'):
            ...
            # print("saaaaaaaaaaaaaaaaaaaaaa",predict_dict)
            # input()
            predict_dict['keys'] = np.array([0,1], dtype=np.int64)
            if 'hr_cam' in predict_dict.keys():
                predict_dict['hr_cam'] = np.nan_to_num(predict_dict['hr_cam'])
            elif 'rw' in predict_dict.keys():
                predict_dict['rw'] = np.nan_to_num(predict_dict['rw'])
            keys = predict_dict['keys']
            # print([type(el) for el in keys])
            # print(keys[np.argmax(cams, axis=0)])
            # print('\n\n\n\n\n')
...
if __name__ == '__main__':
    ...
    if args.mode == 'png':
        loglist = do_python_eval(args.predict_dir, args.gt_dir, name_list, num_cls=num_cls)
        print('mIoU={:.3f}%, IOU BG={:.3f}%, IOU FG={:.3f}% FP={:.4f}, FN={:.4f}'.format(loglist['mIoU'], loglist['background'], loglist['burning'], loglist['fp_all'], loglist['fn_all']))
    elif args.mode == 'rw':
