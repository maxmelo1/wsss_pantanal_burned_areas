...
if __name__ == '__main__':
    ...
    imagenet_mean = [0.041, 0.035, 0.023, 0.126]
    imagenet_std = [0.019, 0.013, 0.01, 0.056]
    ...
    def get_cam(ori_image, scale):
        ...
        test = copy.deepcopy(image).unsqueeze(0).cuda()
        ...
        _, test = model(test, with_cam=True)
        test = F.relu(test)

        t = test.cpu().detach().numpy()
    ...
    with torch.no_grad():
        ...
        for step, (ori_image, image_id, label, gt_mask) in enumerate(dataset):
            ...
            # print('\n', image_id, label, ori_w, ori_h)
            # print(np.unique(get_cam(ori_image, 1).cpu().detach(), return_counts=True))

            keys_cpu = keys.cpu().numpy()
            strided_cams_cpu = strided_cams.cpu().numpy()
            
            if not keys_cpu.size:
                strided_cams = torch.from_numpy(np.zeros(strided_cams_cpu.shape))
            else:                        
                strided_cams = strided_cams[keys]
            strided_cams /= F.adaptive_max_pool2d(strided_cams, (1, 1)) + 1e-5
            
            hr_cams_cpu = hr_cams.cpu().numpy()
            if not keys_cpu.size:
                hr_cams = torch.from_numpy(np.zeros(hr_cams_cpu.shape))
            else:
                hr_cams = hr_cams[keys]
            hr_cams /= F.adaptive_max_pool2d(hr_cams, (1, 1)) + 1e-5

            # print('str-> ', sum(strided_cams_cpu[strided_cams_cpu>0]))
            # print('hr-> ', sum(hr_cams_cpu[hr_cams_cpu>0]))
            # input()
            # save cams
